...
| Protect S-Mode from lower privilege levels (with or without H-extension)
| SR_PMP_004
...
Either address translation and protection or SPMP can be used both with or without the hypervisor extension. For
example, the hypervisor extension with SPMP can support static partitioning
hypervisors commonly used in automotive. A single stage address translation and protection can be used
...
| Implement a system level HW RoT
| SR_ROT_001,
SR_ROT_002
...
the hypervisor configuring address translation and protection and IOMMU translation rules.

NOTE: IOMPT can also be sufficient for protecting Root devices in the sense that
...
[caption="Figure {counter:image}: ", reftext="Figure {image}"]
[title= "GlobalPlatform TEE use cases"]
image::img_ch4_gp-tee.png[]

https://globalplatform.org/[_GlobalPlatform_] (GP) defines technical standards,
...
| (SR_PMP_001, SR_PMP_002) OR (SR_MPT_001, SR_MPT_002), +
  SR_SUD_001 OR SR_SUD_002


| Block resources assigned to TEE domain from access by Normal domain
| SR_SUD_001 OR SR_SUD_002, SR_SUD_003, SR_SUD_004
...
See xref:chapter3.adoc#supervisor_domains[Supervisor Domains] +
See xref:chapter3.adoc#_pmp[PMP and Smepmp] +
See xref:chapter3.adoc#_spmp[SPMP] +
See xref:chapter3.adoc#atp[Address Translation and Protection] +
...
| Use Supervisor domains to enforce isolation between Normal and TEE domains, and to protect M-Mode from other domains
| SR_SUD_001 OR SR_SUD_002, SR_MPT_001, SR_MPT_002

| For a static partition TEE, use PMP, SPMP, address translation and protection or MPT to enforce isolation
between TA in TEE domain.
| SR_PMP_005 OR SR_ATP_003

| For a virtualized TEE, use hypervisor extension
| SR_HYP_001,SR_ATP_001, SR_ATP_002,

| For a virtualized TEE, SPMP or address translation and protection MUST be used to enforce isolation between guest
...
| On a TEE based system, implement a HW RoT
...
a| 
Attestation is used to determine trustworthiness across all components +
...
| Ensure Local sealing for a TA, or a TEE guest is unique to TEE domain and to a
physical instance of a system.

| SR_SUD_003, SR_SLG_002

| Ensure Local sealing for a TA, or a TEE guest is also unique to the TEE guest
or the TA. Local sealing may be layered

| SR_ATP_003 OR SR_PMP_005
|===
...
| Use IOPMP to enforce access rules for devices with a static partition TEE
| SR_IOP_004

| Use IOMPT and IOMMU to enforce access rules for devices
assigned to Normal or TEE domains with a virtualized TEE, and IOPMP to enforce access rules
for Root devices.
...
For a virtualized TEE, IOMPT enforces supervisor domain level access rules
...
* Device assignment (IOPMP/IOMPT integration), static or dynamic
...
| Enable External debug separately for Root domain.
| SR_DBG_001, SR_DBG_002

|  Enable External debug separately for each supervisor domain.
| SR_SUD_007

| Enable External debug by a HW RoT (Root domain external debug)
or by Root domain (supervisor domain external debug).
| SR_DBG_001, SR_SUD_007

| Use Self-hosted debug for debug within a supervisor domain.
| SR_DBG_003

| Enable Self-hosted debug using a higher privileged component.
| SR_DBG_004
...
| Only enable external debug following system reset (part of measuring)
of the affected component.
|SR_DBG_006

| Only enable revealing self-hosted debug following reboot (part of
measuring) of the affected component.
|SR_DBG_007

| Enable Trusted self-hosted debug at runtime (after measuring) of the
affected component, under an application specific governance process.
|SR_DBG_008
|===

These requirements are to guarantee  the system remains attestable.
...
as some mobile clients or edge devices whose design may be constrained by real
...
Confidential workloads require isolation guarantees. RISC-V hardware enforced isolation mechanisms can be used as follows to meet those requirements:
...
| SR_SUD_001 OR SR_SUD_002, SR_SUD_003, SR_SUD_004, SR_ATP_001, SR_ATP_002
...
| Ensure resources assigned to a confidential workload are not accessible by other confidential workloads
without consent.
| SR_ATP_003
...
IOMPT enforces supervisor domain level access rules (physical isolation).
IOMMU enforces guest and TA level access rules (virtualization), supporting
device assignment to a Confidential guest.

NOTE: IOMPT can also be sufficient for protecting Root devices in the sense
...
* Modifying cryptographic memory protection (uniqueness)
* Tagging interrupts, debug accesses, or coherent memory accesses
* Device assignment (IOPMP/IOMPT integration), static or dynamic
...
CoVE defines RISC-V support for
TDISPfootnote:[https://pcisig.com/specifications/]footnote:[https://github.com/riscv-non-isa/riscv-ap-tee-io].
...
These requirements guarantee the system remains attestable.
