...
use-cases can require more. The two domains are notionally labelled _hosting domain_ and _confidential domain_ but they have identical properites. _Workloads_ are run in a domain, and are isolated from each other within that domain. This isolation is managed by a _runtime environment_ or a _domain security manager_.  Switching between domains is managed by a _root domain security manager_.  
Logically isolated domains at the software level are typically reflected in logically isolated domains at the system hardware level, where _devices_ can be assigned or restricted to specific software domains. For example, device DMA transfers can be restricted to memory assigned to a particular domain.
