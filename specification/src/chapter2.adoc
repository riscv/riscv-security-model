...
| RISC-V Worlds
| Hardware World Identifiers for Systyem level isolation
| M, S
...
| Both
| Fine Grained
| HW/SW impactfootnote:[CHERI requires extensive HW modifications, doubles pointer sizes, requires additional SW as well as recompilation and modificationof existing software. This may be offset by performance gains over alternative mechanisms and saving in other state such as PMP.]
