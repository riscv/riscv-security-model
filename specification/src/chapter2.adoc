...
The _Trusted Computing Base (TCB)_ of any system function is the set of
protection mechanisms within a computer system, including hardware,
firmware, and software that are relied upon to enforce a security
policy.
...
Using a HW RoT moves critical functions and assets off an application processor hart to a dedicated and possibly isolated trusted subsystem, which can
provide stronger protection against physical and logical attacks.

If present, the HW RoT acts as a _primary root of trust_ on the system.

NOTE: It is common for secure systems to support multiple trust chains with
their own RoT. For example, a TPM can be a RoT for UEFI
boot flows within a runtime environment while a SIM can be a RoT for
user identity management. +
 +
For the purpose of this document, these should be treated as _secondary roots of
trust_. +
 +
The HW RoT can manage the security life cycle of a secondary RoT (booting etc).
...
| SR_ROT_001
| A system MUST implement a HW RoT
| SR_ROT_002
| A system MUST implement a FW RoT

|===

NOTE: Complex systems such as multiple, out-of-order, cache-coherent harts, require HW RoT. Simple systems such as a single,
in-order, hart may only require a FW RoT
...
| Physical
| Fine Grained
| Reprogramming and Switching overhead, Constrained resourcefootnote:[On switching domains, the PMP must be reprogrammed by M-Mode FW, and PMP entries may be limited in number.]
...
| Physical
| Fine Grained
| Reprogramming and Switching overhead, Constrained resourcefootnote:[On switching domains, the SPMP must be reprogrammed by M-Mode FW, and PMP entries may be limited in number.]
...
| Virtual
| Page Based
| Additional Table Walkfootnote:[The additional table walk required to fetch domain permissions may introduce a performance penalty, or require additional cached state.]
...
| Smmpt, SDID
| Supervisor domains and memory protection tables, building block for confidential computing, trusted execution. +
S-S isolation +
( including hypervisor extension support +
HS - HS isolation )
| M
| Physical
| Page or larger
...
| Both
| Fine Grained
| HW/SW impactfootnote:[CHERI requires extensive HW modifications, doubles pointer sizes, requires additional SW as well as recompilation and modification of existing software. This may be offset by performance gains over alternative mechanisms and saving in other state such as PMP.]
...
| SR_INV_002
| Invasive subsystems MUST be enabled separately for M-Mode &
non-M-Mode software.

| SR_INV_003
| Invasive subsystems MUST be enabled separately for individual domains

|===

NOTE: Fully disabling invasive subsystems, or limiting access in other ways can be an alternative to partitioning by privilege level or domain. +
...
Performance event counters are commonly used for performance management and resource
...
* Allocation of execution resources among harts in a multithreaded system
...
* Logical +
The attacker runs software on the same system as the victim, often as an unprivileged user. These attacks exploit logical or architectural vulnerabilities to compromise privileged software or co-resident applications.

* Physical +
The attacker has physical access to the system hosting the victim process. This enables invasive techniques such as probing, glitching, and disassembly, as well as local side-channel attacks like electromagnetic or power analysis.

* Remote +
The attacker does not have physical or logical access to the victim system and interacts solely over a network. Examples include protocol-level attacks on exposed services and remote side-channel attacks that exploit timing or resource usage observable through network interactions or shared infrastructure.
...
Architectural sandboxing, such as Hardware Fault Isolation.
...
+
For example: +
https://www.cisa.gov/sites/default/files/2023-12/CSAC_TAC_Recommendations-Memory-Safety_Final_20231205_508.pdf[Memory Safety] +
...
| SR_IDN_001
| A platform MUST be securely identifiable
|===

Identifies the immutable part of the platform - immutable hardware,
...
| SR_AUT_002
| A system in security provisioning state MUST only load authorized software.
...
NOTE: When a system in a security provisioning state doesn't restrict loading to only authorized software, other protection measures such as physical access protection, or device registration are required.
...
| SR_UPD_003
| Updates affecting a security platform MUST be deferred.

| SR_UPD_004
| Live update capability MUST ensure suitable governance and be
...
| SR_UPD_006
| System updates MUST be robust against update failures
|===
...
| SR_UPD_007
| System updates, and authorization messages, MUST only be received from
trusted sources.
...
| SR_SLG_001
| Sealed assets MUST only be possible to unseal in a secured state
