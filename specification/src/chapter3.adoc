...
To prevent timing side side channels that may infer the address space layout of supervisor memory, user-mode accesses to supervisor-mode memory must fault in constant time. The Svukte extension ensures load/stores from U-Mode to S-Mode addresses generate page faults independent of the target address, mitigating this timing attack.
