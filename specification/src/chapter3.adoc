...
*See chapter 6 (PMP Enhancements for memory access and execution prevention in Machine mode) of the https://github.com/riscv/riscv-isa-manual/releases/latest[Privileged
ISA] specification.*
...
_This extension is not ratified at the time of writing, but is considered stable enough for inclusion_
...
confidentiality and trustworthyness of the affected components, and must be subject to an attested policy  
...
Depending on use case, IOPMP may be an alternative or addition to IOMPT. For example, a system
...
| SR_AMS_006
| Cryptographically protected architectural metadata storage MUST provide at error detection

| SR_AMS_007
| Cryptographically protected architectural metadata storage MUST provide a integrity protection
